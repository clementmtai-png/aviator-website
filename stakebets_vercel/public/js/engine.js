const Engine = {
    ...
    generateCrashPoint() {
        // Use pure random for demo variety
        // if (typeof getDeterministicCrashPoint !== 'undefined') { ... }

        const now = new Date();
        const mins = now.getMinutes();

        // Golden Hour Logic (Config driven)
        if (CONFIG.GOLDEN_HOUR_ENABLED) {
            const isGoldenWindow = (mins % CONFIG.GOLDEN_HOUR_INTERVAL_MINS) < CONFIG.GOLDEN_HOUR_DURATION_MINS;
            if (isGoldenWindow && Math.random() > 0.5) {
                const min = CONFIG.GOLDEN_HOUR_MIN_MULT;
                const max = CONFIG.GOLDEN_HOUR_MAX_MULT;
        ...
        // Pareto distribution with extra jitter
        const r2 = Math.random();
        const crash = 0.99 / (1 - ((r + r2) / 2));
        const finalCP = Math.min(1000, Math.max(1.00, parseFloat(crash.toFixed(2))));

        // Prevent exact immediate repeats (extremely rare but possible)
        if (finalCP === this.lastCrashPoint) return this.generateCrashPoint();
        this.lastCrashPoint = finalCP;
        return finalCP;
    },
    ...
    addHistory(val) {
        ...
        const type = val < 2 ? 'low' : (val < 10 ? 'mid' : (val < 50 ? 'high' : 'epic'));
        pill.className = `h-pill ${type}`;
    ...
    updateButtonUI(state) {
        ...
        if (state === 'BET') {
            const val = parseFloat(Utils.el('bet-amount').value) || 100.00;
            btn1.innerHTML = `
                <span class="btn-icon">🎰</span>
                <span class="btn-text">BET</span>
                <span class="btn-subtext">${val.toFixed(2)} KES</span>
            `;
            btn1.className = 'bet-btn-main action-bet';
            btn1.disabled = false;
        } else if (state === 'WAITING') {
            btn1.innerHTML = `
                <span class="btn-icon">⏳</span>
                <span class="btn-text">WAITING</span>
            `;
            btn1.disabled = true;
            ...
            btn1.innerHTML = `
                ...
                <span class="btn-subtext">${win} KES</span>
            `;
            btn1.className = 'bet-btn-main action-bet'; // Use bet style or custom cashout style
            btn1.style.background = 'linear-gradient(135deg, #32d74b 0%, #28a745 100%)'; // Green for cashout
            btn1.style.boxShadow = '0 4px 0 #1e7e34';
            btn1.disabled = false;
        }
    },
    ...
    draw() {
        // Use logical dimensions for coordinate calculations
        ...
        // Path calculation
        const progress = Math.min(1.2, (this.multiplier - 1) / 15);
