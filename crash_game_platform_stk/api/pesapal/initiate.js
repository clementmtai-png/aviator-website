...
module.exports = async function handler(req, res) {
  ...
  // Optional: STK Push flag
  const isStk = body.stk === true || body.stk === 'true' || body.stk === '1';
  ...
  try {
    ...
    // If STK Push is requested, we use the SubmitOrderRequest but ensure phone is present
    // Pesapal 3.0 automatically triggers STK if M-PESA is chosen and phone is provided
    // in some configurations, or we might need to specify the channel. 
    // However, the standard SubmitOrderRequest with a redirect is the primary way.
    // For "Direct" STK Push API 3.0, we use a different endpoint if available.
    ...
    if (redirectUrl) {
      return res.status(200).json({
        ...
        order_tracking_id: orderData.order_tracking_id || orderData.orderTrackingId,
        merchant_reference: orderData.merchant_reference || orderData.merchantReference,
      });
    }
    const errMsg = orderData.message || orderData.error_description || orderData.error || 'Submit order failed';
