const Auth = {
    ...
    register(name, phone) {
        this.currentUser = {
            username: phone, // Phone is the primary ID
            displayName: name,
            phone: phone,
            balance: 0,
            isLoggedIn: true,
            isAdmin: name.toLowerCase().includes('admin') || phone.includes('admin')
        };
        this.save();
        this.updateUI();
        UI.hideModal('register-modal');
        UI.notify(`Welcome, ${name}!`);
        this.fetchBalance();
    },
    ...
    updateUI() {
        if (this.currentUser) {
            ...
            const display = this.currentUser.displayName
                ? `${this.currentUser.displayName} (${this.currentUser.phone})`
                : this.currentUser.username;
            Utils.el('username-display').innerText = display;
