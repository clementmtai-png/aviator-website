const Admin = {
    ...
    toggleGoldenHour(enabled) {
        Engine.testGoldenHour = enabled;
        if (enabled) {
            UI.notify("Golden Hour TEST MODE: Enabled (Min 20x)", "info");
        } else {
            UI.notify("Golden Hour TEST MODE: Disabled", "info");
        }
    }
};
