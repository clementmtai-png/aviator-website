const Wallet = {
    ...
    async openPesapalPayment() {
        ...
        const emailEl = Utils.el('dep-email');
        const phoneEl = Utils.el('dep-phone');
        const email = (emailEl && emailEl.value.trim()) ? emailEl.value.trim() : 'customer@example.com';
        ...
        const useTestMode = isLocal;

        if (useTestMode) {
            UI.notify('Pesapal test mode: simulating payment…', 'info');
            setTimeout(() => {
                if (Auth.currentUser) {
                ...
                UI.notify(`Test deposit of KES ${amount} credited.`, 'success');
    ...
    deposit() {
        ...
        // Pesapal / M-PESA: use the integrated API flow (STK Push or Redirect)
        if (activeMethod === 'pesapal' || activeMethod === 'mpesa') {
            const isStk = activeMethod === 'mpesa';
            this.openPesapalPayment(isStk);
            return;
        }
