const Engine = {
    ...
    crash() {
        this.running = false;
        this.crashTime = Date.now() + this.serverTimeOffset;
    ...
    loop() {
        ...
        // Debug: Uncomment to trace loop execution
        // console.log('[Loop] cycleTime:', cycleTime, 'BETTING_TIME:', this.BETTING_TIME, 'isBetting:', cycleTime < this.BETTING_TIME);
        ...
        if (cycleTime < this.BETTING_TIME) {
            ...
            // Allow 2s delay to show crash results
            if (this.crashTime && (now - this.crashTime < 2000)) {
                return this.draw() || requestAnimationFrame(() => this.loop());
            }

            this.multiplier = 1.00;
            this.crashTime = 0; // Reset
    ...
    draw() {
        ...
        // 3. Draw Flight Path (Thick, glowing line like popular crash games)
        const points = Math.min(100, (this.multiplier - 1) * 20);

        if (points > 0) {
            // Create gradient for the line
            const gradient = this.ctx.createLinearGradient(w * 0.05, h * 0.85, w * 0.85, h * 0.2);
            ...
            this.ctx.shadowBlur = this.running ? 15 : 0;
