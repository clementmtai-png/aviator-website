const Engine = {
    ...
    draw() {
        ...
        // Path calculation
        const progress = Math.min(1, (this.multiplier - 1) / 10); // 1.0 to 11.0x covers the width
        const px = startX + progress * (w * 0.8);
        const py = startY - Math.pow(progress, 1.5) * (h * 0.6);

        // Draw flight path (curved line)
        this.ctx.beginPath();
        this.ctx.strokeStyle = this.running ? 'rgba(211, 47, 47, 0.4)' : 'rgba(100, 100, 100, 0.2)';
        ...
        // Calculate angle for airplane (simple slope approximation)
        const angle = Math.atan2(py - startY, px - startX) * 0.5; // Smoothed angle

        this.drawAirplane(px, py, angle);

        // Optional: Draw Flame/Trail
    ...
    drawAirplane(x, y, angle) {
        ...
        this.ctx.beginPath();
        this.ctx.ellipse(8, -2, 4, 2, 0, 0, Math.PI * 2);
        this.ctx.fill();
        ...
        // Propeller (rotating spinning effect)
        const spin = (Date.now() % 100) / 100;
        this.ctx.strokeStyle = 'rgba(255, 255, 255, 0.6)';
        this.ctx.lineWidth = 2;
        this.ctx.beginPath();
        this.ctx.moveTo(18, -8 * Math.sin(spin * Math.PI * 2));
        this.ctx.lineTo(18, 8 * Math.sin(spin * Math.PI * 2));
        this.ctx.stroke();
    ...
    drawTrail(x, y, angle) {
        ...
        const gradient = this.ctx.createLinearGradient(-20, 0, -50, 0);
