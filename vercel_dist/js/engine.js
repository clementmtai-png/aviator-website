const Engine = {
    ...
    generateCrashPoint() {
        ...
        // [USER REQUEST]: "first 2 mins of every hour to have consistently higher than 20"
        if (minutes < 2) {
            // Generate a high multiplier between 20x and 80x
            return parseFloat((20.00 + (Math.random() * 60.00)).toFixed(2));
        }

        // Standard realistic crash algorithm for the rest of the hour
        const r = Math.random();
        // 3% chance of instant crash (house edge)
        if (r < 0.03) return 1.00;

        // Pareto distribution for crash games
