...
async function getToken() {
  ...
  if (!res.ok) {
    throw new Error(`PesaPal Auth Failed (${res.status}): ${text.slice(0, 100)}`);
  }
  try {
    ...
    throw new Error(`PesaPal Auth Invalid JSON: ${text.slice(0, 100)}`);
  }
}
...
async function registerIPN(token, ipnUrl) {
  if (!ipnUrl || ipnUrl.includes('undefined') || ipnUrl.includes('your-project-name')) {
